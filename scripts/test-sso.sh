...
authentik_response=$(curl -s -w "%{http_code}" http://localhost:9000 -o /dev/null || echo "failed")
...
admin_response=$(curl -s -L http://localhost:9000/if/admin/ -w "%{http_code}" -o /dev/null || echo "failed")
if [ "$admin_response" = "failed" ]; then
    echo "❌ Authentik admin interface not accessible"
else
    echo "✅ Authentik admin interface is accessible"
fi

# Test 3: Check WordPress availability
echo ""
echo "3️⃣ Testing WordPress availability..."
wp_response=$(curl -s -w "%{http_code}" http://localhost:8080 -o /dev/null || echo "failed")
...
owui_response=$(curl -s -w "%{http_code}" http://localhost:3000 -o /dev/null || echo "failed")
...
auth_endpoint=$(curl -s -w "%{http_code}" http://localhost:9000/application/o/authorize/ -o /dev/null || echo "failed")
if [ "$auth_endpoint" = "failed" ]; then
    echo "⚠️  OAuth authorization endpoint not accessible (may require parameters)"
else
    echo "✅ OAuth authorization endpoint is accessible"
fi

# Test token endpoint
token_endpoint=$(curl -s -w "%{http_code}" http://localhost:9000/application/o/token/ -o /dev/null || echo "failed")
if [ "$token_endpoint" = "failed" ]; then
    echo "⚠️  OAuth token endpoint not accessible (may require parameters)"
else
    echo "✅ OAuth token endpoint is accessible"
fi

# Test userinfo endpoint
userinfo_endpoint=$(curl -s -w "%{http_code}" http://localhost:9000/application/o/userinfo/ -o /dev/null || echo "failed")
if [ "$userinfo_endpoint" = "failed" ]; then
    echo "⚠️  OAuth userinfo endpoint not accessible (may require authentication)"
else
    echo "✅ OAuth userinfo endpoint is accessible"
fi

# Test 6: Check if WordPress has OpenID Connect plugin ready
echo ""
echo "6️⃣ Checking WordPress OpenID Connect plugin readiness..."
wp_plugins_check=$(curl -s http://localhost:8080/wp-admin/plugins.php 2>/dev/null || echo "failed")
